...
class Barrel:
    def __init__(self, remainder):
        ...
        self.gen = self.generator_of_barrel()

    def generator_of_barrel(self):
        a = [x for x in range(1, self.remainder + 1)]
        random.shuffle(a)
        for i, y in enumerate(a):
            print("Новый бочонок: {} (осталось {})".format(y, self.remainder - (i + 1)))
            yield y
...
def main(): #Основной интерфейс
    ...
    while True:
        ...
        player1.get_card(ticket.card_user)
        player2.get_card(ticket.card_comp)

        inp_user = input("Зачеркнуть цифру? (y/n)")
        if inp_user == "y":
            if player1.search(ticket.card_user, num_barrel):
                continue
            else:
                print("Игра окончена!")
                sys.exit(1)
        if inp_user == "n":
            if player1.search(ticket.card_user, num_barrel):
                print('Игра окончена!')
                sys.exit(1)
            elif player2.search(ticket.card_comp, num_barrel):
                continue
